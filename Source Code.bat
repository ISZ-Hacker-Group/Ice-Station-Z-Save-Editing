...
cls
echo.
echo ID = Item ID List (This Cosnole Window)
echo.
echo E = Exit
echo.
echo M = Main Menu
echo.
echo SM = Simple ID List (Uses A Web-Browser)
echo.
:choice2
set /P c=Type an Option. [SM/ID/E/M]:
if /I "%c%" EQU "ID" goto :pg1
if /I "%c%" EQU "M" goto :beg
if /I "%c%" EQU "E" goto :ext
if /I "%c%" EQU "SM" goto :simpled
goto :choice2
...
::starts microsoft edge with item id list.
:simpled
cls
sleep 0.3
echo.
echo STARTING MS-EDGE (Simple ID Version)
echo.
start msedge.exe "%CD%\_metaD\_data0\_part\edit_id.html"
echo.
cls
sleep 0.3
goto :beg
